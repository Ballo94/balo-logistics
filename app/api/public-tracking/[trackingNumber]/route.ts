...
export async function GET(_request: Request, context: { params: Promise<{ trackingNumber: string }> }) {
  ...
  try {
    const [historyResult, snapshotResult, snapshotStopsResult, documentsResult, communicationsResult] = await Promise.all([
      ...
      supabase.from("shipment_route_stops").select("id, position, name, country, city, stop_type, code, onward_transport, estimated_duration_hours, estimated_distance_km, expected_arrival_offset, expected_departure_offset, default_status_text, logistics_location_id").eq("shipment_id", shipmentId).order("position"),
...
async function loadLegacyJourney(supabase: AdminClient, routeTemplateId: string | null) {
  ...
  const [templateResult, stopsResult] = await Promise.all([
    ...
    supabase.from("route_stops").select("id, route_template_id, position, name, country, city, stop_type, code, onward_transport, estimated_duration_hours, estimated_distance_km, expected_arrival_offset, expected_departure_offset, default_status_text, logistics_location_id").eq("route_template_id", routeTemplateId).order("position"),
  ]);

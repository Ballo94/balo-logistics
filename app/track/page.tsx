...
function ShipmentSummary({ shipment, lastUpdated }: { shipment: Shipment; lastUpdated: string }) { const progress = shipmentProgress(shipment.shipment_status); return <section className="overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-[0_22px_55px_-28px_rgba(15,23,42,.32)]" aria-labelledby="shipment-summary-title"><div className="relative overflow-hidden bg-[#071a33] p-6 text-white sm:p-8 lg:p-10"><div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,.24),transparent_52%)]"/><div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[0.68rem] font-extrabold uppercase tracking-[0.2em] text-cyan-300">Shipment summary</p><h2 id="shipment-summary-title" className="mt-2 break-all text-2xl font-black sm:text-3xl">{shipment.tracking_number}</h2><p className="mt-2 text-sm text-blue-200">Last updated {formatDate(lastUpdated, true)}</p></div><span className={`w-fit rounded-2xl px-5 py-3 text-sm font-black uppercase tracking-wider ring-1 ring-inset ${summaryStatusClasses(shipment.shipment_status)}`}>{shipment.shipment_status || "Pending"}</span></div></div><div className="grid gap-px bg-slate-200 sm:grid-cols-2 lg:grid-cols-5"><SummaryItem label="Tracking number" value={shipment.tracking_number}/><SummaryItem label="Shipment status" value={shipment.shipment_status || "Pending"}/><SummaryItem label="Current location" value={shipment.current_location || "Update pending"}/><SummaryItem label="Estimated delivery" value={formatDate(shipment.estimated_delivery) || "To be confirmed"}/><SummaryItem label="Transport mode" value={shipment.transport_mode || "Freight"}/></div><div className="px-6 py-5 sm:px-8"><div className="flex items-center justify-between text-sm font-bold"><span className="text-slate-700">Overall progress</span><span className="text-blue-700">{progress}%</span></div><div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full bg-gradient-to-r from-blue-700 via-cyan-500 to-emerald-500 transition-[width] duration-700" style={{ width: `${progress}%` }}/></div></div></section>; }
...
function ShipmentInformation({ shipment }: { shipment: Shipment }) { return <section aria-labelledby="shipment-information-title"><SectionHeading eyebrow="Consignment details" title="Shipment Information" id="shipment-information-title"/><div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3"><InformationCard icon={<RouteIcon/>} label="Origin" value={shipment.origin_country}/><InformationCard icon={<PinIcon/>} label="Destination" value={shipment.destination_country}/><InformationCard icon={<CargoIcon/>} label="Weight" value={shipment.weight_kg == null ? "Not provided" : `${shipment.weight_kg} kg`}/><InformationCard icon={<PackageIcon/>} label="Quantity" value={shipment.package_count == null ? "Not provided" : `${shipment.package_count}${shipment.package_type ? ` ${shipment.package_type}` : ""}`}/><InformationCard icon={<ShieldIcon/>} label="Insurance" value={shipment.declared_value == null ? "Not provided" : `Declared value ${money(shipment.declared_value)}`}/><InformationCard icon={<DocumentIcon/>} label="Incoterms" value="Not provided"/><InformationCard icon={<ReceiverIcon/>} label="Receiver" value={shipment.receiver_name || "Not provided"}/><InformationCard icon={<SupplierIcon/>} label="Supplier" value={shipment.client_name || "Not provided"}/><InformationCard icon={<ContainerIcon/>} label="Container number" value="Not provided"/></div></section>; }
...
function TrustIndicators({ shipment }: { shipment: Shipment }) { const badges = ["Shipment Verified", "Customs Documentation Complete", "Shipment Insured", "Secure Handling", "Tracking Updated Automatically"]; return <InfoCard eyebrow="Balo assurance" title="Trust Indicators" icon={<ShieldIcon/>}><ul className="grid gap-3">{badges.map((badge) => <li key={badge} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold ${badge === "Shipment Insured" && shipment.declared_value == null ? "bg-slate-50 text-slate-500" : "bg-emerald-50/70 text-emerald-900"}`}><span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-white ${badge === "Shipment Insured" && shipment.declared_value == null ? "bg-slate-400" : "bg-emerald-600"}`}><CheckSmallIcon/></span>{badge}</li>)}</ul></InfoCard>; }
...
function DocumentIcon() { return <SvgIcon><path d="M6 3h8l4 4v14H6z"/><path d="M14 3v5h5M9 13h6M9 17h6"/></SvgIcon>; }

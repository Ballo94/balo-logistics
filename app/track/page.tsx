...
type TransportKind = "air" | "sea" | "road" | "other";
function transportKind(mode: string | null): TransportKind { const value = mode?.toLowerCase() ?? ""; if (value.includes("air")) return "air"; if (value.includes("sea") || value.includes("ocean")) return "sea"; if (value.includes("road") || value.includes("truck")) return "road"; return "other"; }
function logisticsLocation(country: string, kind: TransportKind) { const key = country.trim().toLowerCase(); const locations: Record<string, { air: string; sea: string; road: string }> = { "south africa": { air: "OR Tambo International Airport (JNB)", sea: "Durban Port (ZADBN)", road: "Johannesburg Distribution Centre" }, namibia: { air: "Hosea Kutako International Airport (WDH)", sea: "Port of Walvis Bay (NAWVB)", road: "Windhoek Distribution Centre" }, china: { air: "Shanghai Pudong International Airport (PVG)", sea: "Port of Shanghai (CNSHA)", road: "Shanghai Distribution Centre" }, "united arab emirates": { air: "Dubai International Airport (DXB)", sea: "Jebel Ali Port (AEJEA)", road: "Dubai Distribution Centre" }, france: { air: "Paris Charles de Gaulle Airport (CDG)", sea: "Port of Le Havre (FRLEH)", road: "Paris Distribution Centre" }, singapore: { air: "Singapore Changi Airport (SIN)", sea: "Port of Singapore (SGSIN)", road: "Singapore Distribution Centre" } }; return locations[key]?.[kind === "other" ? "road" : kind] ?? (kind === "air" ? `${country} International Airport` : kind === "sea" ? `Port of ${country}` : kind === "road" ? `${country} Distribution Centre` : country); }
function transportProfile(shipment: Shipment) { const kind = transportKind(shipment.transport_mode); const origin = logisticsLocation(shipment.origin_country, kind); const destination = logisticsLocation(shipment.destination_country, kind); const current = shipment.current_location || (kind === "sea" ? "Ocean position update pending" : "Location update pending"); return { kind, origin, current, destination, nextStop: destination, label: kind === "air" ? "Air Freight" : kind === "sea" ? "Sea Freight (FCL)" : kind === "road" ? "Road Freight" : shipment.transport_mode || "Freight", detailLabel: kind === "air" ? "Flight stage" : kind === "sea" ? "Vessel name" : kind === "road" ? "Current highway or border" : "Current stage", detailValue: kind === "air" ? shipment.shipment_status || "Stage update pending" : kind === "sea" ? shipment.courier_name || "Not assigned" : kind === "road" ? shipment.current_location || "Route update pending" : shipment.shipment_status || "Update pending" }; }
      ...
      <div className="relative mx-auto -mt-9 max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
        ...
        {shipment && <div className="track-results mt-5 space-y-4" aria-live="polite">
          ...
          <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(19rem,.75fr)]">
            <ShipmentTimeline shipmentStatus={shipment.shipment_status} history={history} originCountry={shipment.origin_country} destinationCountry={shipment.destination_country} originLabel={transportProfile(shipment).origin} destinationLabel={transportProfile(shipment).destination} transportMode={shipment.transport_mode} />
...
function HeroNetworkOverlay() { return <svg aria-hidden="true" viewBox="0 0 1400 520" preserveAspectRatio="xMidYMid slice" className="pointer-events-none absolute inset-0 h-full w-full opacity-[0.075]"><g fill="none" stroke="#bfdbfe" strokeWidth="1"><ellipse cx="700" cy="260" rx="430" ry="185"/><path d="M270 260h860M330 180h740M330 340h740M700 75c-125 90-125 280 0 370M700 75c125 90 125 280 0 370"/></g><g fill="none" stroke="#f6c945" strokeWidth="1.5" strokeDasharray="7 10"><path d="M205 330Q390 105 555 275T890 205T1210 315"/><path d="M335 145Q590 405 1030 125"/></g><g fill="#f6c945"><circle cx="205" cy="330" r="4"/><circle cx="555" cy="275" r="4"/><circle cx="890" cy="205" r="4"/><circle cx="1210" cy="315" r="4"/><circle cx="335" cy="145" r="3"/><circle cx="1030" cy="125" r="3"/></g><g fill="#bfdbfe"><path d="m420 118 38-14-13 13 18 7-43-2 12-6Z"/><path d="M1045 366h54l-8 15h-34l-12-15Zm12-13h29v13h-29Z"/><path d="M188 322c0-7 5-12 12-12s12 5 12 12c0 9-12 20-12 20s-12-11-12-20Zm8 0a4 4 0 1 0 8 0 4 4 0 0 0-8 0Z"/></g></svg>; }

function HeroJourneyCard({ shipment }: { shipment: Shipment }) { const progress = shipmentProgress(shipment.shipment_status); const profile = transportProfile(shipment); return <aside className="rounded-[1.25rem] border border-white/15 bg-[#06172d]/75 p-4 shadow-[0_20px_50px_-28px_rgba(0,0,0,.7)] backdrop-blur-xl sm:p-5" aria-label="Shipment journey summary"><div className="flex items-center justify-between gap-4"><div><p className="text-[0.6rem] font-extrabold uppercase tracking-[0.16em] text-yellow-300">Shipment Journey</p><p className="mt-1 break-all text-xs font-bold text-white">{shipment.tracking_number}</p></div><div className="flex items-center gap-2 text-yellow-300"><ModeIcon mode={shipment.transport_mode}/><span className="text-2xl font-black">{progress}%</span></div></div><dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3"><HeroJourneyItem label="Origin" value={profile.origin}/><HeroJourneyItem label="Current location" value={profile.current}/><HeroJourneyItem label="Destination" value={profile.destination}/><HeroJourneyItem label="Transport mode" value={profile.label}/><HeroJourneyItem label="Estimated arrival" value={formatDate(shipment.estimated_delivery) || "To be confirmed"}/><HeroJourneyItem label={profile.detailLabel} value={profile.detailValue}/></dl><div className="mt-4 h-1 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-[#f6c945]" style={{ width: `${progress}%` }}/></div></aside>; }
...
function ShipmentInformation({ shipment }: { shipment: Shipment }) { const profile = transportProfile(shipment); return <section aria-labelledby="shipment-information-title"><SectionHeading eyebrow="Route and cargo" title="Route Information" id="shipment-information-title"/><div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3"><InformationCard icon={<ModeIcon mode={shipment.transport_mode}/>} label="Origin" value={profile.origin}/><InformationCard icon={<PinIcon/>} label="Destination" value={profile.destination}/><InformationCard icon={<SupplierIcon/>} label="Supplier" value={shipment.client_name || "Not provided"}/><InformationCard icon={<ReceiverIcon/>} label="Receiver" value={shipment.receiver_name || "Not provided"}/><InformationCard icon={<CargoIcon/>} label="Weight" value={shipment.weight_kg == null ? "Not provided" : `${shipment.weight_kg} kg`}/><InformationCard icon={<PackageIcon/>} label="Quantity" value={shipment.package_count == null ? "Not provided" : `${shipment.package_count}${shipment.package_type ? ` ${shipment.package_type}` : ""}`}/><InformationCard icon={<DimensionsIcon/>} label="Dimensions" value="Not provided"/><InformationCard icon={<ShieldIcon/>} label="Insurance" value={shipment.declared_value == null ? "Not provided" : `Active · ${money(shipment.declared_value)}`}/><InformationCard icon={<ModeIcon mode={shipment.transport_mode}/>} label="Transport mode" value={profile.label}/><InformationCard icon={<ContainerIcon/>} label="Container number" value="Not provided"/><InformationCard icon={<SealIcon/>} label="Seal number" value="Not provided"/></div></section>; }
...
function RouteOverview({ shipment }: { shipment: Shipment }) { const profile = transportProfile(shipment); const points = [{ label: "Origin", value: profile.origin, state: "complete" }, { label: "Current location", value: profile.current, state: "current" }, { label: "Destination", value: profile.destination, state: "future" }] as const; return <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_16px_40px_-28px_rgba(15,23,42,.32)] sm:p-6" aria-labelledby="route-overview-title"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-blue-700"><ModeIcon mode={shipment.transport_mode}/></span><div><p className="text-[0.62rem] font-extrabold uppercase tracking-[0.18em] text-blue-600">Shipment journey</p><h2 id="route-overview-title" className="text-lg font-black tracking-tight">Route Overview</h2></div></div><div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_14rem]"><ol className="grid gap-0 sm:grid-cols-3" aria-label="Shipment route">{points.map((point, index) => <li key={point.label} className="relative flex min-w-0 gap-3 pb-5 last:pb-0 sm:block sm:pb-0 sm:text-center">{index < points.length - 1 && <span aria-hidden="true" className="absolute left-[13px] top-7 h-[calc(100%-1rem)] border-l-2 border-dashed border-blue-200 sm:left-1/2 sm:top-[13px] sm:h-0 sm:w-full sm:border-l-0 sm:border-t-2"/>}<span className={`relative z-10 block h-7 w-7 shrink-0 rounded-full border-[6px] border-white shadow-[0_0_0_1px_rgba(148,163,184,.4)] sm:mx-auto ${point.state === "complete" ? "bg-[#f6c945]" : point.state === "current" ? "bg-blue-600 ring-4 ring-blue-100" : "bg-slate-300"}`}/><div className="min-w-0 sm:mt-3"><p className="text-[0.6rem] font-extrabold uppercase tracking-[0.13em] text-slate-400">{point.label}</p><p className="mt-1 break-words text-sm font-black text-slate-800">{point.value}</p></div></li>)}</ol><div className="grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-4 lg:grid-cols-1"><div><p className="text-[0.6rem] font-extrabold uppercase tracking-wider text-slate-400">Next stop</p><p className="mt-1 text-sm font-bold text-slate-800">{profile.nextStop}</p></div><div><p className="text-[0.6rem] font-extrabold uppercase tracking-wider text-slate-400">Transport</p><p className="mt-1 flex items-center gap-2 text-sm font-bold text-blue-700"><ModeIcon mode={shipment.transport_mode}/>{profile.label}</p></div></div></div></section>; }

function LiveStatusCard({ shipment }: { shipment: Shipment }) { const progress = shipmentProgress(shipment.shipment_status); const profile = transportProfile(shipment); return <article className="sticky top-5 relative overflow-hidden rounded-[1.5rem] border border-blue-900/10 bg-[#071a33] p-5 text-white shadow-[0_22px_55px_-28px_rgba(7,26,51,.7)] sm:p-6"><div aria-hidden="true" className="absolute -right-16 -top-20 h-48 w-48 rounded-full bg-blue-500/20 blur-3xl"/><div className="relative"><div className="flex items-start justify-between gap-4"><div><p className="text-[0.62rem] font-extrabold uppercase tracking-[0.18em] text-yellow-300">Live status</p><h2 className="mt-1 flex items-center gap-2 text-lg font-black tracking-tight"><ModeIcon mode={shipment.transport_mode}/>Current Progress</h2></div><span className="text-4xl font-black tracking-[-0.06em] text-yellow-300">{progress}%</span></div><dl className="mt-5 grid gap-3"><LiveField label="Current location" value={profile.current}/><LiveField label="Next destination" value={profile.nextStop}/><LiveField label="Estimated arrival" value={formatDate(shipment.estimated_delivery) || "To be confirmed"}/>{profile.kind === "sea" && <LiveField label="Current ocean position" value={shipment.current_location || "Position update pending"}/>}</dl><div className="mt-5"><div className="flex justify-between text-xs font-bold text-blue-200"><span>Shipment progress</span><span>{progress}% complete</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-[#f6c945] transition-[width] duration-700" style={{ width: `${progress}%` }}/></div></div></div></article>; }

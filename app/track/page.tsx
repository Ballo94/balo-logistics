  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f4f7fb] text-slate-950">
      ...
      <div className="relative mx-auto -mt-12 max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        ...
        {shipment && <div className="track-results mt-7 space-y-5" aria-live="polite">
          <RouteOverview shipment={shipment} />
          <ShipmentSummary shipment={shipment} lastUpdated={latestUpdate?.created_at ?? shipment.created_at} />
          <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(19rem,.75fr)]">
          ...
          <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(20rem,.8fr)]">
...
function TrackingHero({ shipment }: { shipment: Shipment | null }) { return <section className="relative overflow-hidden bg-[#071e3b] px-4 pb-24 pt-28 text-white sm:px-6 sm:pb-28 sm:pt-32 lg:px-8 lg:pb-28 lg:pt-36">
  ...
  <div className={`track-reveal relative mx-auto grid max-w-7xl items-center gap-8 ${shipment ? "lg:grid-cols-[1.12fr_.88fr]" : ""}`}><div className={shipment ? "" : "mx-auto max-w-4xl text-center"}><div className={`mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-blue-950/35 px-4 py-2 text-xs font-extrabold uppercase tracking-[0.2em] text-cyan-100 backdrop-blur ${shipment ? "" : "mx-auto"}`}><span className="h-2 w-2 animate-pulse rounded-full bg-[#f6c945]"/>Live shipment tracking</div><h1 className="text-4xl font-black leading-[1.02] tracking-[-0.05em] sm:text-5xl lg:text-6xl">Your shipment, in sight.</h1><p className={`mt-4 max-w-2xl text-sm leading-6 text-blue-100/90 sm:text-base ${shipment ? "" : "mx-auto"}`}>Your shipment is currently travelling through our international logistics network with secure milestone updates from origin to delivery.</p>
    <ul className={`mt-6 flex max-w-4xl flex-wrap gap-x-5 gap-y-2 text-xs font-semibold text-blue-100 sm:text-sm ${shipment ? "" : "mx-auto justify-center"}`}>{["Secure tracking", "International logistics", "Air & Sea freight", "Door-to-door delivery"].map((item) => <li key={item} className="flex items-center gap-2"><CheckCircleIcon />{item}</li>)}</ul></div>
    {shipment && <HeroJourneyCard shipment={shipment}/>}
  </div>
</section>; }

function HeroJourneyCard({ shipment }: { shipment: Shipment }) { const progress = shipmentProgress(shipment.shipment_status); return <aside className="rounded-[1.5rem] border border-white/15 bg-[#06172d]/75 p-5 shadow-[0_24px_60px_-28px_rgba(0,0,0,.7)] backdrop-blur-xl sm:p-6" aria-label="Shipment journey summary"><div className="flex items-center justify-between gap-4"><div><p className="text-[0.62rem] font-extrabold uppercase tracking-[0.18em] text-yellow-300">Shipment Journey</p><p className="mt-1 break-all text-sm font-bold text-white">{shipment.tracking_number}</p></div><span className="text-3xl font-black text-yellow-300">{progress}%</span></div><dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-4"><HeroJourneyItem label="Origin" value={shipment.origin_country}/><HeroJourneyItem label="Current location" value={shipment.current_location || "Update pending"}/><HeroJourneyItem label="Destination" value={shipment.destination_country}/><HeroJourneyItem label="Estimated delivery" value={formatDate(shipment.estimated_delivery) || "To be confirmed"}/></dl><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-[#f6c945]" style={{ width: `${progress}%` }}/></div></aside>; }

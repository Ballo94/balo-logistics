...
import { canonicalizeShipmentStatus, deriveShipmentState } from "../lib/shipment-state";
...
export default function CustomerShipmentDetails({ shipment, onClose, onRead }: { shipment: CustomerShipment; onClose: () => void; onRead: () => void }) {
  ...
  const previousRouteStatus = canonicalizeShipmentStatus(shipment.shipment_status) === "exception" ? history.find((item) => canonicalizeShipmentStatus(item.status) !== "exception")?.status ?? null : null;

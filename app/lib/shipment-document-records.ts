...
export const SHIPMENT_DOCUMENT_TYPES = ["Commercial Invoice", "Packing List", "Air Waybill", "Bill of Lading", "Customs Declaration", "Delivery Note", "Insurance Certificate", "Import Permit", "Export Permit", "Proof of Delivery", "Other"] as const;
...
export type ShipmentDocument = { id: number; shipment_id: number; document_name: string; document_type: ShipmentDocumentType; file_url: string; file_size: number; visible_to_customer: boolean; uploaded_at: string; created_by?: string | null; notes?: string | null };

const ADMIN_DOCUMENT_COLUMNS = "id, shipment_id, document_name, document_type, file_url, file_size, visible_to_customer, uploaded_at, created_by, notes";
...
export function formatFileSize(bytes: number) { if (bytes < 1024) return `${bytes} B`; if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(bytes < 10240 ? 1 : 0)} KB`; return `${(bytes / (1024 * 1024)).toFixed(1)} MB`; }

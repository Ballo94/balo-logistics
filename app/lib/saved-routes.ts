...
export type RouteLocationMetadata = { country_code: string; location_type: string; secondary_code: string | null };
export type SavedRouteStop = { id: string; route_template_id: string; position: number; name: string; country: string; city: string; stop_type: SavedRouteStopType; code: string | null; operational_notes: string | null; onward_transport: JourneyLegMode | null; estimated_duration_hours?: number | null; estimated_distance_km?: number | null; leg_internal_notes?: string | null; expected_arrival_offset?: number | null; expected_departure_offset?: number | null; default_status_text?: string | null; logistics_location_id?: string | null; logistics_location?: RouteLocationMetadata | RouteLocationMetadata[] | null };
...
export function buildJourneyFromSavedRoute(template: SavedRouteTemplate, orderedStops: SavedRouteStop[]): RouteJourney | null {
  ...
  const legs: RouteLeg[] = stops.slice(0, -1).map((stop, sequence) => ({ id: `saved-leg-${stop.id}`, sequence, transportMode: segmentMode(stop, template), displayMode: stop.onward_transport ?? undefined, estimatedDurationHours: stop.estimated_duration_hours ?? null, estimatedDistanceKm: stop.estimated_distance_km ?? null, internalNotes: stop.leg_internal_notes ?? null, origin: locations[sequence], destination: locations[sequence + 1], checkpointIds: checkpoints.filter((item) => item.location.id === locations[sequence].id || item.location.id === locations[sequence + 1].id).map((item) => item.id), references: {} }));
...
export async function loadSavedRoute(routeTemplateId: string) {
  const [{ data: template, error: templateError }, { data: stops, error: stopsError }] = await Promise.all([
    ...
    supabase.from("route_stops").select("id, route_template_id, position, name, country, city, stop_type, code, operational_notes, onward_transport, estimated_duration_hours, estimated_distance_km, leg_internal_notes, expected_arrival_offset, expected_departure_offset, default_status_text, logistics_location_id, logistics_location:logistics_locations(country_code, location_type, secondary_code)").eq("route_template_id", routeTemplateId).order("position"),
  ]);

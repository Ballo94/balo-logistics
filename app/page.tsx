  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f5f7fa] text-slate-950">
      <section className="relative isolate min-h-[54rem] overflow-hidden bg-[#1676b5] text-white sm:min-h-[56rem] lg:min-h-[52rem]">
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 border-t border-white/15 bg-[#07558f]/58 backdrop-blur-md"><div className="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-white/15 px-4 sm:grid-cols-4 sm:px-6 lg:px-8"><Metric value="120+" label="Trade routes"/><Metric value="24/7" label="Shipment visibility"/><Metric value="Air • Sea" label="Multimodal freight"/><Metric value="50+" label="Partner network"/></div></div>
      </section>
...
function Metric({ value, label }: { value: string; label: string }) { return <div className="px-3 py-5 text-center sm:px-5"><p className="whitespace-nowrap text-[clamp(1.75rem,4vw,3rem)] font-black leading-none tracking-tight text-yellow-300">{value}</p><p className="mt-1 text-[0.65rem] font-bold uppercase tracking-wider text-blue-200">{label}</p></div>; }

...
import { applyRouteRecommendations, COORDINATES_UNAVAILABLE_MESSAGE, effectiveEstimate, hasUsableVerifiedCoordinates, recommendRouteStopLeg } from "../lib/route-journey-estimates";
...
export default function RouteLibraryEditor({ value, saving, dragged, onDragged, onChange, onClose, onSave, onDuplicate }: Props) {
  ...
  const calculateRecommendation = (index: number, selectedMode?: EditableRouteStop["onward_transport"]) => {
    ...
    const recommendation = recommendRouteStopLeg(stop, destination, selectedMode);
    updateStop(index, {
  ...
  const move = (from: number, to: number) => {
    ...
    patch({ stops: applyRouteRecommendations(stops) });
  };
    ...
    <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl bg-[#f4f7fb] shadow-2xl">
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="grid gap-4">
          ...
          <LogisticsLocationSelector key={value.transport_mode} transportMode={value.transport_mode} onAdd={(stop) => { const index = insertAt === "end" ? value.stops.length : Math.min(insertAt, value.stops.length); const stops = [...value.stops]; stops.splice(index, 0, stop); const fallbackMode = value.transport_mode === "Multimodal" ? null : value.transport_mode; if (index > 0 && !stops[index - 1].onward_transport) stops[index - 1] = { ...stops[index - 1], onward_transport: fallbackMode }; if (process.env.NODE_ENV === "development" && stops.length > 1) { const departure = stops[0]; const arrival = stops[1]; const departureLocation = routeLocationMetadata(departure); const arrivalLocation = routeLocationMetadata(arrival); console.debug("[BALO ROUTE DEBUG] SELECTED DEPARTURE LOCATION", { id: departure.logistics_location_id, name: departure.name, latitude: departureLocation?.latitude ?? null, longitude: departureLocation?.longitude ?? null, latitudeType: typeof departureLocation?.latitude, longitudeType: typeof departureLocation?.longitude, location: departureLocation }); console.debug("[BALO ROUTE DEBUG] SELECTED ARRIVAL LOCATION", { id: arrival.logistics_location_id, name: arrival.name, latitude: arrivalLocation?.latitude ?? null, longitude: arrivalLocation?.longitude ?? null, latitudeType: typeof arrivalLocation?.latitude, longitudeType: typeof arrivalLocation?.longitude, location: arrivalLocation }); console.debug("[BALO ROUTE DEBUG] ROUTE STOP / LEG STATE", { legIndex: 0, legId: departure.id, departureStop: departure, arrivalStop: arrival, transportMode: departure.onward_transport, originCoordinates: { latitude: departureLocation?.latitude ?? null, longitude: departureLocation?.longitude ?? null }, destinationCoordinates: { latitude: arrivalLocation?.latitude ?? null, longitude: arrivalLocation?.longitude ?? null } }); } patch({ stops: applyRouteRecommendations(stops) }); setInsertAt("end"); }}/>
          ...
          <section className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            <ol className="mt-4 grid gap-3">
              {value.stops.map((stop, index) => { const metadata = routeLocationMetadata(stop); const destinationStop = value.stops[index + 1]; const destinationMetadata = destinationStop ? routeLocationMetadata(destinationStop) : null; const coordinatesReady = hasUsableVerifiedCoordinates(metadata) && hasUsableVerifiedCoordinates(destinationMetadata); const liveRecommendation = destinationStop ? recommendRouteStopLeg(stop, destinationStop) : null; const recommendationDistance = liveRecommendation?.distanceKm; const recommendationDuration = liveRecommendation?.durationHours; const recommendationConfidence = liveRecommendation?.confidence; const recommendationReason = destinationStop && !coordinatesReady ? COORDINATES_UNAVAILABLE_MESSAGE : liveRecommendation?.metadata.unavailableReason; const effectiveDuration = effectiveEstimate(stop.estimated_duration_hours, recommendationDuration); const effectiveDistance = effectiveEstimate(stop.estimated_distance_km, recommendationDistance); if (process.env.NODE_ENV === "development" && index === 0 && destinationStop) { console.debug("[BALO ROUTE DEBUG] SELECTED DEPARTURE LOCATION", { id: stop.logistics_location_id, name: stop.name, latitude: metadata?.latitude ?? null, longitude: metadata?.longitude ?? null, latitudeType: typeof metadata?.latitude, longitudeType: typeof metadata?.longitude, location: metadata }); console.debug("[BALO ROUTE DEBUG] SELECTED ARRIVAL LOCATION", { id: destinationStop.logistics_location_id, name: destinationStop.name, latitude: destinationMetadata?.latitude ?? null, longitude: destinationMetadata?.longitude ?? null, latitudeType: typeof destinationMetadata?.latitude, longitudeType: typeof destinationMetadata?.longitude, location: destinationMetadata }); console.debug("[BALO ROUTE DEBUG] ROUTE STOP / LEG STATE", { legIndex: index, legId: stop.id, departureStop: stop, arrivalStop: destinationStop, transportMode: stop.onward_transport, originCoordinates: { latitude: metadata?.latitude ?? null, longitude: metadata?.longitude ?? null }, destinationCoordinates: { latitude: destinationMetadata?.latitude ?? null, longitude: destinationMetadata?.longitude ?? null } }); console.debug("[BALO ROUTE DEBUG] JOURNEY LEG INPUT RENDER", { legIndex: index, legId: stop.id, recommendationDuration, recommendationDistance, administratorDurationHours: stop.estimated_duration_hours ?? null, administratorDistanceKm: stop.estimated_distance_km ?? null, effectiveDuration, effectiveDistance }); } return <li key={stop.id} draggable onDragStart={() => onDragged(index)} onDragEnd={() => onDragged(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragged != null) move(dragged, index); onDragged(null); }} className={`rounded-xl border bg-slate-50 p-3 transition ${dragged === index ? "border-blue-400 opacity-60" : "border-slate-200"}`}>
                ...
                {index < value.stops.length - 1 && <section className="mt-3 rounded-xl border border-blue-100 bg-white p-3" aria-label={`Journey leg ${index + 1}`}>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Field label="Transport Mode"><select value={stop.onward_transport ?? ""} onChange={(event) => calculateRecommendation(index, event.target.value ? event.target.value as NonNullable<EditableRouteStop["onward_transport"]> : null)} className="workflow-field"><option value="">Choose mode</option><option>Road</option><option>Air</option><option>Sea</option><option>Rail</option><option>Courier</option><option>Internal Transfer</option></select></Field><Field label="Duration (hours)"><input type="number" min="0" step="0.25" value={effectiveDuration ?? ""} onChange={(event) => updateStop(index, { estimated_duration_hours: event.target.value === "" ? null : Number(event.target.value) })} className="workflow-field"/></Field><Field label="Distance (km)"><input type="number" min="0" step="0.1" value={effectiveDistance ?? ""} onChange={(event) => updateStop(index, { estimated_distance_km: event.target.value === "" ? null : Number(event.target.value) })} className="workflow-field"/></Field><Field label="Internal Notes"><input value={stop.leg_internal_notes ?? ""} onChange={(event) => updateStop(index, { leg_internal_notes: event.target.value })} className="workflow-field"/></Field></div>

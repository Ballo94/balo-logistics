    ...
    <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl bg-[#f4f7fb] shadow-2xl">
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="grid gap-4">
          ...
          <LogisticsLocationSelector transportMode={value.transport_mode} onAdd={(stop) => { const index = insertAt === "end" ? value.stops.length : Math.min(insertAt, value.stops.length); const stops = [...value.stops]; stops.splice(index, 0, stop); patch({ stops }); setInsertAt("end"); }}/>
          ...
          <section className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            <ol className="mt-4 grid gap-3">
              {value.stops.map((stop, index) => { const metadata = routeLocationMetadata(stop); return <li key={stop.id} draggable onDragStart={() => onDragged(index)} onDragEnd={() => onDragged(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragged != null) move(dragged, index); onDragged(null); }} className={`rounded-xl border bg-slate-50 p-3 transition ${dragged === index ? "border-blue-400 opacity-60" : "border-slate-200"}`}>
                ...
                {index < value.stops.length - 1 && <section className="mt-3 rounded-xl border border-blue-100 bg-white p-3" aria-label={`Journey leg ${index + 1}`}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[0.58rem] font-black uppercase tracking-[0.14em] text-blue-600">Journey Leg {index + 1}</p><p className="mt-1 text-xs font-black text-slate-800">{stop.name} <span className="px-1 text-blue-500">→</span> {value.stops[index + 1].name}</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[0.6rem] font-black text-slate-600">Order {index + 1}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Field label="Transport Mode"><select value={stop.onward_transport ?? ""} onChange={(event) => updateStop(index, { onward_transport: event.target.value as EditableRouteStop["onward_transport"] })} className="workflow-field"><option value="">Choose mode</option><option>Road</option><option>Air</option><option>Sea</option><option>Rail</option><option>Courier</option><option>Internal Transfer</option></select></Field><Field label="Duration (hours)"><input type="number" min="0" step="0.25" value={stop.estimated_duration_hours ?? ""} onChange={(event) => updateStop(index, { estimated_duration_hours: event.target.value === "" ? null : Number(event.target.value) })} className="workflow-field"/></Field><Field label="Distance (km)"><input type="number" min="0" step="0.1" value={stop.estimated_distance_km ?? ""} onChange={(event) => updateStop(index, { estimated_distance_km: event.target.value === "" ? null : Number(event.target.value) })} className="workflow-field"/></Field><Field label="Internal Notes"><input value={stop.leg_internal_notes ?? ""} onChange={(event) => updateStop(index, { leg_internal_notes: event.target.value })} className="workflow-field"/></Field></div></section>}
              </li>; })}
            </ol>

...
import {
  ...
  SHIPMENT_DOCUMENT_LIFECYCLE_STATUSES, SHIPMENT_DOCUMENT_TYPES, storagePathFromUrl, effectiveDocumentLifecycleStatus,
  type ShipmentDocument, type ShipmentDocumentDirection, type ShipmentDocumentLifecycleStatus, type ShipmentDocumentType,
} from "../lib/shipment-document-records";
import { supabase } from "../lib/supabase";
import { DOCUMENT_REPLACEMENT_REASON_PRESETS, DOCUMENT_REQUIRED_FOR_PRESETS, generateDocumentRequestMessage, generateReplacementExplanation, type DocumentReplacementReasonPreset } from "../lib/document-customer-messages";
...
const LIFECYCLE_LABELS: Record<ShipmentDocumentLifecycleStatus, string> = { requested: "Requested", received: "Received", submitted: "Submitted", completed: "Completed", replacement_required: "Replacement Required" };
...
type RequiredForPreset = (typeof DOCUMENT_REQUIRED_FOR_PRESETS)[number];
...
function inferRequiredForPreset(value: string | null | undefined): RequiredForPreset { return DOCUMENT_REQUIRED_FOR_PRESETS.includes(value as RequiredForPreset) ? value as RequiredForPreset : "Other"; }
...
export default function ShipmentDocumentsManager({ shipmentId, transportMode = "", cargoDescription = "" }: { shipmentId: number; transportMode?: string; cargoDescription?: string }) {
  ...
  const [lifecycleStatus, setLifecycleStatus] = useState<ShipmentDocumentLifecycleStatus>("completed");
  const [requiredFor, setRequiredFor] = useState("");
  const [requiredForPreset, setRequiredForPreset] = useState<RequiredForPreset>("Shipment documentation");
  const [replacementReason, setReplacementReason] = useState("");
  const [replacementReasonPreset, setReplacementReasonPreset] = useState<DocumentReplacementReasonPreset>("Updated document required");
  const [replacementMessageEdited, setReplacementMessageEdited] = useState(false);
  ...
  const customerLifecycle = isRequest || direction === "Received from Customer" || Boolean(editingDocument && effectiveDocumentLifecycleStatus(editingDocument));
  const availableLifecycleStatuses = editingDocument?.file_url ? SHIPMENT_DOCUMENT_LIFECYCLE_STATUSES.filter((status) => status !== "requested") : (["requested"] as const);
  ...
  const filtered = useMemo(() => { const query = search.trim().toLowerCase(); return documents.filter((item) => !query || item.document_name.toLowerCase().includes(query) || item.document_type.toLowerCase().includes(query) || item.document_direction.toLowerCase().includes(query) || (effectiveDocumentLifecycleStatus(item) ? LIFECYCLE_LABELS[effectiveDocumentLifecycleStatus(item)!].toLowerCase().includes(query) : false)); }, [documents, search]);

  function chooseFile(selected: File | null) { setFile(selected); if (selected && !name.trim()) setName(selected.name); setError(""); setMessage(""); }
  function startEdit(document: ShipmentDocument) { const inferredCargo = inferCargoContext(cargoDescription); const storedReplacement = document.replacement_reason ?? ""; setEditingId(document.id); setName(document.document_name); setType(document.document_type); setDirection(document.document_direction); setRequestMode(document.document_direction === "Document Request"); setRequestSearch(""); setRequestCargo(inferredCargo); setCustomCommodity(inferredCargo === "Other / Custom Commodity" ? cargoDescription.trim() : ""); setRequestTransportMode(inferTransportMode(transportMode)); setCustomDocumentMode(false); setCustomDocumentName(""); setNotes(document.notes ?? ""); setLifecycleStatus(effectiveDocumentLifecycleStatus(document) ?? "completed"); setRequiredFor(document.required_for ?? ""); setRequiredForPreset(inferRequiredForPreset(document.required_for)); setReplacementReason(storedReplacement); setReplacementReasonPreset("Other"); setReplacementMessageEdited(Boolean(storedReplacement)); setVisible(document.visible_to_customer); setFile(null); setError(""); setMessage(""); formRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" }); }
  function reset() { setEditingId(null); setFile(null); setName(""); setType("Commercial Invoice"); setDirection("Sent to Customer"); setRequestMode(false); setRequestSearch(""); setCustomCommodity(""); setCustomDocumentMode(false); setCustomDocumentName(""); setNotes(""); setLifecycleStatus("completed"); setRequiredFor(""); setRequiredForPreset("Shipment documentation"); setReplacementReason(""); setReplacementReasonPreset("Updated document required"); setReplacementMessageEdited(false); setVisible(true); setError(""); }
  function beginRequest() { const inferredCargo = inferCargoContext(cargoDescription); reset(); setDirection("Document Request"); setLifecycleStatus("requested"); setRequiredFor("Shipment documentation"); setRequiredForPreset("Shipment documentation"); setRequestMode(true); setRequestCargo(inferredCargo); setCustomCommodity(inferredCargo === "Other / Custom Commodity" ? cargoDescription.trim() : ""); setRequestTransportMode(inferTransportMode(transportMode)); setVisible(true); setMessage("Choose the document you would like the customer to provide."); formRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" }); }
  ...
  function chooseRequiredFor(preset: RequiredForPreset) { setRequiredForPreset(preset); setRequiredFor(preset === "Other" ? "" : preset); }
  function chooseReplacementReason(preset: DocumentReplacementReasonPreset) { setReplacementReasonPreset(preset); setReplacementMessageEdited(false); setReplacementReason(generateReplacementExplanation(name, preset)); }
  function changeReplacementMessage(value: string) { setReplacementReason(value); setReplacementMessageEdited(true); }
  function changeDocumentName(value: string) { setName(value); if (lifecycleStatus === "replacement_required" && replacementReasonPreset !== "Other" && !replacementMessageEdited) setReplacementReason(generateReplacementExplanation(value, replacementReasonPreset)); }
  ...
  async function save() {
    const existing = editingDocument;
    ...
    const nextLifecycle: ShipmentDocumentLifecycleStatus = isRequest ? "requested" : customerLifecycle ? lifecycleStatus : "completed";
    const now = new Date().toISOString();
    const base = { document_name: name.trim(), document_type: type, document_direction: direction, notes: notes.trim() || null, visible_to_customer: visible, lifecycle_status: nextLifecycle, required_for: customerLifecycle ? requiredFor.trim() || null : null, replacement_reason: nextLifecycle === "replacement_required" ? replacementReason.trim() || null : null, submitted_at: nextLifecycle === "submitted" ? existing?.submitted_at ?? now : null, completed_at: nextLifecycle === "completed" ? existing?.completed_at ?? now : null };
    if (editingId) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_14px_36px_-28px_rgba(15,23,42,.35)]">
    ...
    <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,.82fr)_minmax(0,1.18fr)]">
      <div ref={formRef} className="grid content-start gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
        ...
        {!requestMode && <label><span className={labelClass}>Document Name *</span><input value={name} onChange={(event) => changeDocumentName(event.target.value)} className={inputClass} /></label>}
        ...
        {requestMode && <label><span className={labelClass}>Request Name *</span><input value={name} onChange={(event) => changeDocumentName(event.target.value)} className={inputClass} /></label>}
        {requestMode ? <div><span className={labelClass}>Document Direction</span><div className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-2.5 text-sm font-black text-blue-700">Document Request</div></div> : <label><span className={labelClass}>Document Direction</span><select value={direction} onChange={(event) => { const next = event.target.value as ShipmentDocumentDirection; setDirection(next); setLifecycleStatus(next === "Document Request" ? "requested" : next === "Received from Customer" ? "received" : "completed"); if (next === "Document Request") { setRequestMode(true); setVisible(true); } }} className={inputClass}>{direction === "Internal / Admin" && <option>Internal / Admin</option>}{DOCUMENT_DIRECTIONS.map((option) => <option key={option} disabled={Boolean(editingId && documents.find((item) => item.id === editingId)?.file_url && option === "Document Request")}>{option}</option>)}</select></label>}
        {customerLifecycle && <><label><span className={labelClass}>Required For</span><select value={requiredForPreset} onChange={(event) => chooseRequiredFor(event.target.value as RequiredForPreset)} className={inputClass}>{DOCUMENT_REQUIRED_FOR_PRESETS.map((preset) => <option key={preset}>{preset}</option>)}</select>{requiredForPreset === "Other" && <input value={requiredFor} onChange={(event) => setRequiredFor(event.target.value)} className={`${inputClass} mt-2`} placeholder="Enter the actual purpose" />}</label>{isRequest && <div className="rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5"><span className={labelClass}>Suggested Customer Message</span><p className="text-xs font-semibold leading-5 text-slate-700">{generateDocumentRequestMessage(name, requiredFor)}</p></div>}<label><span className={labelClass}>Lifecycle Status</span><select value={lifecycleStatus} disabled={!editingId || !editingDocument?.file_url} onChange={(event) => { const next = event.target.value as ShipmentDocumentLifecycleStatus; setLifecycleStatus(next); if (next === "replacement_required" && !replacementReason.trim()) chooseReplacementReason("Updated document required"); }} className={`${inputClass} disabled:bg-slate-100 disabled:text-slate-500`}>{availableLifecycleStatuses.map((status) => <option key={status} value={status}>{LIFECYCLE_LABELS[status]}</option>)}</select><span className="mt-1 block text-[0.66rem] text-slate-500">Customer uploads automatically move requests to Received. Later changes are admin-controlled.</span></label>{lifecycleStatus === "replacement_required" && <div><label><span className={labelClass}>Replacement Reason</span><select value={replacementReasonPreset} onChange={(event) => chooseReplacementReason(event.target.value as DocumentReplacementReasonPreset)} className={inputClass}>{DOCUMENT_REPLACEMENT_REASON_PRESETS.map((preset) => <option key={preset}>{preset}</option>)}</select></label><label className="mt-2 block"><span className={labelClass}>Customer Replacement Explanation</span><textarea rows={4} value={replacementReason} onChange={(event) => changeReplacementMessage(event.target.value)} className={inputClass} placeholder="Explain why another copy is needed" /></label>{replacementReasonPreset !== "Other" && <button type="button" onClick={() => chooseReplacementReason(replacementReasonPreset)} className="mt-1 text-[0.66rem] font-black text-blue-700 hover:text-blue-800">Regenerate suggested explanation</button>}</div>}</>}
      ...
      <div className="min-w-0"><div className="mb-2 flex flex-col gap-2 sm:flex-row"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, type, direction, or status" className={`${inputClass} h-10 flex-1`} /><button type="button" onClick={() => setAscending((value) => !value)} className="h-10 rounded-xl border border-slate-200 px-3 text-xs font-black text-slate-700">Upload date {ascending ? "↑" : "↓"}</button></div><div className="overflow-hidden rounded-xl border border-slate-200">{!filtered.length ? <p className="px-4 py-8 text-center text-xs font-semibold text-slate-500">No matching shipment documents.</p> : <ul className="divide-y divide-slate-100">{filtered.map((document) => <AdminDocumentRow key={document.id} document={document} busy={busy} onPreview={() => void openDocument(document)} onDownload={() => void openDocument(document, true)} onEdit={() => startEdit(document)} onDelete={() => void remove(document)} />)}</ul>}</div></div>
    </div>
  </section>;
}
...
function AdminDocumentRow({ document, busy, onPreview, onDownload, onEdit, onDelete }: { document: ShipmentDocument; busy: boolean; onPreview: () => void; onDownload: () => void; onEdit: () => void; onDelete: () => void }) {
  const lifecycle = effectiveDocumentLifecycleStatus(document);
  return <li className="px-3 py-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-50 text-blue-700"><DocumentIcon /></span><div className="min-w-0"><p className="truncate text-xs font-black text-slate-800">{document.document_name}</p><p className="text-[0.66rem] font-semibold text-blue-700">{document.document_type}</p><div className="mt-1 flex flex-wrap gap-1.5"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[0.6rem] font-bold text-slate-600">{document.document_direction}</span>{lifecycle && <span className={`rounded-full px-2 py-0.5 text-[0.6rem] font-black ${lifecycle === "replacement_required" || lifecycle === "requested" ? "bg-amber-100 text-amber-800" : lifecycle === "completed" ? "bg-emerald-100 text-emerald-800" : "bg-blue-100 text-blue-800"}`}>{LIFECYCLE_LABELS[lifecycle]}</span>}</div><p className="mt-1 text-[0.64rem] text-slate-500">{formatDate(document.uploaded_at)} · {formatFileSize(document.file_size)} · <span className={document.visible_to_customer ? "text-emerald-700" : "text-amber-700"}>{document.visible_to_customer ? "Customer visible" : "Internal only"}</span></p>{document.required_for && <p className="mt-1 text-[0.66rem] font-semibold text-slate-600">Required for: {document.required_for}</p>}{document.replacement_reason && lifecycle === "replacement_required" && <p className="mt-1 text-[0.66rem] font-semibold text-amber-800">Replacement reason: {document.replacement_reason}</p>}{document.notes && <p className="mt-1 text-[0.66rem] italic text-slate-500">{document.notes}</p>}</div></div><div className="flex shrink-0 flex-wrap gap-1.5">{document.file_url && <><button type="button" onClick={onPreview} className={actionClass}>Preview</button><button type="button" onClick={onDownload} className={actionClass}>Download</button></>}<button type="button" onClick={onEdit} className={`${actionClass} text-blue-700`}>Edit</button><button type="button" disabled={busy} onClick={onDelete} className={`${actionClass} text-red-700`}>Delete</button></div></div></li>;
}

...
export default function CreateShipment() {
  ...
  function validate() {
    ...
    if (!selectedRouteId) next.route_template_id = "Select a saved route from the Route Library.";
  ...
  function reset() { setForm(emptyForm); setSelectedRouteId(""); setSelectedRouteName(""); setErrors({}); setSuccess(""); }
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="create-shipment-title">
    ...
    <div className="grid gap-4 p-4 sm:p-5">
      ...
      <FormSection number="01" title="Shipment"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4"><Field label="Tracking Number" error={errors.tracking_number}><div className="flex gap-2"><input value={form.tracking_number} onChange={(event) => updateForm("tracking_number", event.target.value.toUpperCase())} placeholder="Generated automatically" className="workflow-field uppercase"/><button type="button" onClick={() => updateForm("tracking_number", generateTrackingNumber())} className="rounded-xl border border-blue-200 px-3 text-xs font-black text-blue-700 hover:bg-blue-50">Generate</button></div></Field><Field label="Saved Route" error={errors.route_template_id} required><select value={selectedRouteId} disabled={loadingRoute} onChange={(event) => void selectRoute(event.target.value)} className="workflow-field"><option value="">Select Route Library template</option>{routeTemplates.map((route) => <option key={route.id} value={route.id}>{route.name}</option>)}</select></Field><Field label="Transport Mode" error={errors.transport_mode} required><select value={form.transport_mode} disabled={Boolean(selectedRouteId)} onChange={(event) => updateForm("transport_mode", event.target.value)} className="workflow-field disabled:bg-slate-100 disabled:text-slate-600">{TRANSPORT_MODES.map((mode) => <option key={mode}>{mode}</option>)}</select></Field><Field label="Estimated Delivery" error={errors.estimated_delivery}><input type="date" value={form.estimated_delivery} onChange={(event) => updateForm("estimated_delivery", event.target.value)} className="workflow-field"/></Field>{form.transport_mode === "Sea" && <Field label="Vessel Name"><input value={form.vessel_name} onChange={(event) => updateForm("vessel_name", event.target.value)} placeholder="Enter vessel name" className="workflow-field"/></Field>}</div>{selectedRouteName && <p className="mt-3 text-xs font-bold text-blue-700">Selected manual route: {selectedRouteName}. Its exact ordered stops will be preserved with this shipment.</p>}</FormSection>

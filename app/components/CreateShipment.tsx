...
import { requiresReceiverDeliveryAddress } from "../lib/shipment-current-location";
...
const emptyForm = {
  ...
  seal_number: "",
  declared_value: "",
  insurance_status: "not_specified",
};
...
export default function CreateShipment() {
  ...
  useEffect(() => { if (!success) return; const timer = window.setTimeout(() => setSuccess(""), 6000); return () => window.clearTimeout(timer); }, [success]);
  ...
  async function createShipment() {
    ...
    if (requiresReceiverDeliveryAddress(selectedRoute.journey) && !form.receiver_address.trim()) {
      setErrors({ receiver_address: "Enter the receiver's final delivery address for this door-to-door route." });
      setSaving(false); return;
    }
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="create-shipment-title">
    ...
    <div className="grid gap-4 p-4 sm:p-5">
      ...
      {success && <div role="status" className="fixed right-4 top-4 z-[100] flex max-w-sm flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-800 shadow-xl"><span>{success}</span><button type="button" onClick={reset} className="rounded-lg bg-emerald-700 px-3 py-2 text-xs font-black text-white">Create another shipment</button></div>}

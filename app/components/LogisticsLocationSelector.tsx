...
export default function LogisticsLocationSelector({ transportMode, onAdd }: { transportMode: string; onAdd: (stop: EditableRouteStop) => void }) {
  ...
  const [country, setCountry] = useState(""); const [type, setType] = useState<LogisticsLocationType | "">(""); const [query, setQuery] = useState("");
  ...
  const [message, setMessage] = useState(""); const [searching, setSearching] = useState(false);
  ...
  useEffect(() => {
    // Country options are queried from production Supabase data.
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (!country && !type && query.trim().length < 2) { setResults([]); setSearching(false); return; }
      setSearching(true); setMessage("");
      const transportTypes = transportFilter === "All" ? undefined : TRANSPORT_TYPES[transportFilter];
      void searchLocations({ country, type, types: type ? undefined : transportTypes, query, verified: "verified", pageSize: 40 }).then((result) => { setResults(result.data); setSearching(false); if (result.error) setMessage(result.error.message); });
    }, 180);
    return () => window.clearTimeout(timer);
  }, [country, query, transportFilter, type]);
  ...
  return <section className="rounded-2xl border border-blue-100 bg-blue-50/40 p-4" aria-labelledby="add-location-title">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <label><Label>Country</Label><select value={country} onChange={(event) => { setCountry(event.target.value); setSelected(null); }} className="field"><option value="">All countries</option>{countries.map((item) => <option key={item.countryCode} value={item.country}>{countryFlag(item.countryCode)} {item.country}</option>)}</select></label>
    ...
    <div className="mt-3 max-h-96 overflow-y-auto" aria-live="polite">
      ...
      {!searching && (country || type || query.trim().length >= 2) && results.length === 0 && <p className="rounded-xl border border-dashed border-slate-300 bg-white p-4 text-xs font-bold text-slate-500">No verified locations match these filters.</p>}
      {!searching && grouped.map(({ group, locations }) => <section key={group} className="mb-3 last:mb-0"><h5 className="sticky top-0 z-10 border-b border-blue-100 bg-blue-50/95 px-2 py-2 text-[0.62rem] font-black uppercase tracking-[0.14em] text-blue-800 backdrop-blur">{group} <span className="text-blue-400">{locations.length}</span></h5><div className="grid gap-2 pt-2">{locations.map((location) => <button type="button" key={location.id} onClick={() => setSelected(location)} className={`flex items-center justify-between gap-3 rounded-xl border p-3 text-left ${selected?.id === location.id ? "border-blue-500 bg-blue-50 ring-2 ring-blue-100" : "border-slate-200 bg-white hover:border-blue-200"}`}><span className="flex min-w-0 items-center gap-3"><span className="text-xl" aria-label={location.country}>{countryFlag(location.country_code)}</span><span className="min-w-0"><span className="block truncate text-xs font-black text-slate-800">{location.name}{location.code ? ` (${location.code})` : ""}</span><span className="mt-1 block text-[0.65rem] font-semibold text-slate-500">{locationTypeLabel(location.location_type, location.name)} · {location.city || "City not listed"}, {location.country}</span>{location.secondary_code && <span className="mt-0.5 block text-[0.6rem] font-bold text-blue-600">Official codes: {[location.code, location.secondary_code].filter(Boolean).join(" / ")}</span>}</span></span><span className="rounded-full bg-emerald-50 px-2 py-1 text-[0.58rem] font-black uppercase text-emerald-700">Verified</span></button>)}</div></section>)}
    </div>
